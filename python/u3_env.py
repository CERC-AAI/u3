...
class U3Environment(UnityEnvironment):
	...
	def __init__(
		...
		super(U3Environment, self).__init__(file_name=file_name, worker_id=worker_id, base_port=base_port, seed=seed, no_graphics=no_graphics, timeout_wait=timeout_wait, side_channels=side_channels )
...
class U3Wrapper(UnityToGymWrapper):
	...
	def __init__(
		...
		super(U3Wrapper, self).__init__(unity_env=unity_env, uint8_visual=uint8_visual, flatten_branched=flatten_branched, allow_multiple_obs=allow_multiple_obs)
...
def create_environment(task):	
	...
	env = U3Wrapper(unity_env, environmentChannel, flatten_branched = True, uint8_visual=True)
	env.seed(task)
